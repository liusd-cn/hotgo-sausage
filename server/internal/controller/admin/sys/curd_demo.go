...
func (c *cCurdDemo) ImportTemplate(ctx context.Context, req *curddemo.ImportTemplateReq) (res *curddemo.ImportTemplateRes, err error) {
	err = service.SysCurdDemo().ImportTemplate(ctx, &req.CurdDemoImportTemplateInp)
	return
}

// Import 导入CURD列表数据
func (c *cCurdDemo) Import(ctx context.Context, req *curddemo.ImportReq) (res *curddemo.ImportRes, err error) {
	data, err := service.SysCurdDemo().Import(ctx, &req.CurdDemoImportInp)
	if err != nil {
		return
	}

	res = new(curddemo.ImportRes)
	res.CurdDemoImportModel = data
	return
}