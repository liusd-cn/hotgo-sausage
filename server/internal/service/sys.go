...
type (
	...
	ISysBook interface {
		...
		Model(ctx context.Context, option ...*handler.Option) *gdb.Model
		// List 获取书籍列表
		List(ctx context.Context, in *sysin.BookListInp) (list []*sysin.BookListModel, totalCount int, err error)
		// Export 导出书籍
		Export(ctx context.Context, in *sysin.BookListInp) (err error)
		// Edit 修改/新增书籍
		Edit(ctx context.Context, in *sysin.BookEditInp) (err error)
		// Delete 删除书籍
		Delete(ctx context.Context, in *sysin.BookDeleteInp) (err error)
		// View 获取书籍指定信息
		View(ctx context.Context, in *sysin.BookViewInp) (res *sysin.BookViewModel, err error)
	}
	...
	ISysCurdDemo interface {
		...
		ImportTemplate(ctx context.Context, in *sysin.CurdDemoImportTemplateInp) (err error)
		// Import 导入CURD列表数据
		Import(ctx context.Context, in *sysin.CurdDemoImportInp) (res *sysin.CurdDemoImportModel, err error)
	}
...
func SysBook() ISysBook {
	if localSysBook == nil {
		panic("implement not found for interface ISysBook, forgot register?")
	}
	return localSysBook
}

func RegisterSysBook(i ISysBook) {
	localSysBook = i
}
