...
// CurdDemoImportInp 导入CURD列表
type CurdDemoImportInp struct {
	File string `json:"file" v:"required#请上传Excel文件" dc:"Excel文件路径"`
}

func (in *CurdDemoImportInp) Filter(ctx context.Context) (err error) {
	return
}

// CurdDemoImportModel 导入结果返回
type CurdDemoImportModel struct {
	SuccessCount int      `json:"successCount" dc:"成功导入数量"`
	FailCount    int      `json:"failCount"    dc:"失败数量"`
	FailDetails  []string `json:"failDetails"  dc:"失败详情"`
}

// CurdDemoImportTemplateInp 下载导入模板
type CurdDemoImportTemplateInp struct{}

func (in *CurdDemoImportTemplateInp) Filter(ctx context.Context) (err error) {
	return
}

type CurdDemoImportTemplateModel struct{}